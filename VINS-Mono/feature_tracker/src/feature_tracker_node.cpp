...
{
  //是否是第一帧，是则修改状态位，并设置第一帧时间和最新帧时间为获取时间
    if(first_image_flag)
    {
    ...
    //不是第一帧的情况下， 如果当前传入帧的时间比最新时间不多于1s或者仍然小于最新的时间
    //判断相机获取的帧是不是冲突了，一般都是按序传入，如果是的话就返回并置为第一帧
...
int main(int argc, char **argv)
{
    ...
    //config->euroc->euroc_config.yaml中的一些配置参数
    readParameters(n);

    //在这里NUM_OF_CAM设置成常量1，只有一个摄像头（单目），读取相机内参。
    for (int i = 0; i < NUM_OF_CAM; i++)
...
//订阅话题和发布话题，监听IMAGE_TOPIC（/cam0/image_raw），
//有图像发布到这个话题上的时候，执行回调函数，
//这里直接进入到img_callback函数中接收图像，前端视觉的算法基本在这个回调函数中。
